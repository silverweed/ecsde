...
use ecs_engine::events::evt_register::{Event_Callback_Data, wrap_cb_data, with_cb_data};
...
impl Ground_Collision_Calculation_System {
    ...
    pub fn init(&mut self, engine_state: &mut Engine_State) {
        engine_state.systems.evt_register.subscribe::<Evt_Entity_Destroyed>(Box::new(
        |entity, to_recalc| {
    ...
    pub fn update(&mut self, world: &mut Ecs_World) {
        with_cb_data(&mut self.entities_to_recalc, |to_recalc: &mut Vec<Entity>| {
            for &entity in to_recalc.iter() {
                ...
                for &i in &directions::square_directions() {
                    ...
                    if world.is_valid_entity(e) {
                        ...
                        let Vec2i{x: width, y: height} = renderable.rect.size();
                        if !world.has_component::<Collider>(e) {
                            world.add_component(e, Collider{
                                shape: Collision_Shape::Rect { width: width as f32, height: height as f32 },
                                is_static: true,
                                ..Default::default()
                            });
                        }
