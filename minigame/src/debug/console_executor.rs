...
pub fn execute(cmdline: &str, game_state: &mut Game_State) -> Option<(String, Color)> {
    match parse_cmd(cmdline) {
        Ok(cmd) => execute_command(cmd, game_state),
        Err(err) => Some((
...
fn execute_command(cmd: Console_Cmd, game_state: &mut Game_State) -> Option<(String, Color)> {
    match cmd {
        Console_Cmd::Quit => {
        ...
        Console_Cmd::Get_Cfg_Var { name } => Some((
            format!(
                "{} = {:?}",
                name,
                game_state.config.read_cfg(String_Id::from(name.as_str()))
            ),
            colors::WHITE,
        )),
