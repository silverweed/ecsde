...
impl Component_Manager {
    ...
    #[inline]
    pub fn map_component<T: 'static, R, F>(&self, entity: Entity, mut f: F) -> Option<R>
    where
        F: FnMut(&T) -> R,
    {
        if let Some(storage) = self.read_component_storage::<T>() {
            Some(f(storage.get(entity)?))
        } else {
            None
        }
    }
    ...
    #[inline]
    pub fn try_write_component<T: 'static, R, F>(&self, entity: Entity, mut f: F) -> bool
    where
        F: FnMut(&mut T),
    {
        if let Some(mut storage) = self.write_component_storage::<T>() {
            if let Some(data) = storage.get_mut(entity) {
                f(data);
                true
            } else {
                false
