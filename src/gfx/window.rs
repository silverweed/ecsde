...
pub fn create_render_window(target_size: (u32, u32), title: &str) -> Window {
	let mut window = sfgfx::RenderWindow::new(
		sfwin::VideoMode::new(target_size.0, target_size.1, sfwin::VideoMode::desktop_mode().bits_per_pixel),
		title,
	...
	while let Some(event) = win.poll_event() {
		match event {
			...
			sfwin::Event::Resized {
				...
				win.set_view(&keep_ratio(&sfsys::Vector2u::new(width, height), &window.target_size));
			},
			_ => ()
		}
	}
